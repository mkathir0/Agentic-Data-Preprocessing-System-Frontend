...
export function SpecialText({
  ...
  const shouldAnimate = inView ? isInView : !triggerOnHover;
  const [hasStarted, setHasStarted] = useState(() => !inView && !triggerOnHover && delay <= 0);
  const text = children;
  const [displayText, setDisplayText] = useState<string>(
    triggerOnHover ? text : " ".repeat(text.length)
  );
  ...
  useEffect(() => {
    if (shouldAnimate && !hasStarted && !triggerOnHover) {
      clearStartTimeout();
      if (delay <= 0) {
        startAnimation();
        return;
  ...
  }, [shouldAnimate, hasStarted, delay, triggerOnHover]);
